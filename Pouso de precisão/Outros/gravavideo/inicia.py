...
raw = cv2.VideoWriter('raw-'+timestr+'.avi',fourcc, 10.0, (640,480))
processed = cv2.VideoWriter('detected-'+timestr+'.avi',fourcc, 10.0, (640,480))
...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
        ...
        # Escreve a imagem raw
        raw.write(img)
        
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
                # Escreve a imagem processada
        ...
        if ids is not None:
                ...
                aruco.drawAxis(gray, cameraMatrix, distCoeffs, rvec[0], mediaPos, CT_t)
                #print(somaPos)
                ...
                logfile = open('log - '+timestr+'.txt','a') 
                logfile.write(str(round(time.time()*1000))+', '+str(round(mediaPos[0]*100,2))+', '+str(round(mediaPos[1]*100,2))+', '+str(round(mediaPos[2]*100,2))+', '+ str(round(mediaAngulos[0],2))+', '+ str(round(mediaAngulos[1],2))+ ', '+ str(round(mediaAngulos[2],2))+ '\n') 
                logfile.close() 
        
        processed.write(cv2.cvtColor(gray, cv2.COLOR_GRAY2BGR))
        # Escreve a imagem processada
        #processed.write(gray)
